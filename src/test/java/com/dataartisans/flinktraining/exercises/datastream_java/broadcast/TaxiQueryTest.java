...
public class TaxiQueryTest extends TaxiRideTestBase<Tuple2<String, String>> {
    ...
    public void nearbyAvailableTaxis() throws Exception {
        ...
        TestStringSource queries =
            new TestStringSource(String.format(Locale.US, "ride.getEuclideanDistance(%f, %f) < 1.0", momaLon, momaLat));
    ...
    private TaxiRide testRide(long rideId, Boolean isStart, DateTime startTime, DateTime endTime, float startLon,
        float startLat, float endLon, float endLat, long taxiId) {
        return new TaxiRide(rideId, isStart, startTime, endTime, startLon, startLat, endLon, endLat, (short)1, taxiId,
            0);
    }
    ...
    private TaxiRide endRide(TaxiRide started, DateTime endTime, float endLon, float endLat) {
        return testRide(started.rideId, false, started.startTime, endTime, started.startLon, started.startLat, endLon,
            endLat, started.taxiId);
    }
