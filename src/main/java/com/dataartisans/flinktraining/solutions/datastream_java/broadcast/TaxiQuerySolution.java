 ...
 *
 * On that socket stream, type java expressions to be matched against both the stored state and newly arriving rides.
 * These expressions have a "ride" and the current "watermark" in scope.
 *
 * Examples:
 *
 * true -- match everything: dump all stored state and match all incoming rides false -- match nothing: stop emitting
 * any results ride.isStart && (watermark - ride.getEventTime()) > 100 * 60000 -- match ongoing rides that started more
 * than 100 minutes ago !ride.isStart && ride.getEuclideanDistance(-74, 41) < 10.0 -- match rides that end within 10km
...
public class TaxiQuerySolution extends ExerciseBase {
    ...
    final static MapStateDescriptor queryDescriptor = new MapStateDescriptor<>("queries",
        BasicTypeInfo.STRING_TYPE_INFO, TypeInformation.of(ExpressionEvaluator.class));
    ...
    public static void main(String[] args) throws Exception {
        ...
        BroadcastStream<String> queryStream =
            env.addSource(stringSourceOrTest(new SocketTextStreamFunction("localhost", 9999, "\n", -1)))
                .assignTimestampsAndWatermarks(new QueryStreamAssigner()).broadcast(queryDescriptor);

        // connect the two streams and process queries
        DataStream<Tuple2<String, String>> results =
            rides.keyBy((TaxiRide ride) -> ride.taxiId).connect(queryStream).process(new QueryProcessor());
    ...
    public static class QueryProcessor
        extends KeyedBroadcastProcessFunction<Long, TaxiRide, String, Tuple2<String, String>> {
        ...
        @Override
        public void processElement(TaxiRide ride, ReadOnlyContext ctx, Collector<Tuple2<String, String>> out)
            throws Exception {
        ...
        @Override
        public void processBroadcastElement(String query, Context ctx, Collector<Tuple2<String, String>> out)
            throws Exception {
            ...
            ctx.applyToKeyedState(rideDescriptor, new KeyedStateFunction<Long, ValueState<TaxiRide>>() {
                ...
                public void process(Long taxiId, ValueState<TaxiRide> taxiState) throws Exception {
                    ...
                    if (evaluateBooleanExpression(ee, ride, ctx.currentWatermark())) {
                        out.collect(
                            new Tuple2<>("PBE@" + timeFormatter.print(ctx.currentWatermark()), ride.toString()));
                    }
                }
            });
        }
        ...
        private boolean evaluateBooleanExpression(ExpressionEvaluator ee, TaxiRide ride, long watermark)
