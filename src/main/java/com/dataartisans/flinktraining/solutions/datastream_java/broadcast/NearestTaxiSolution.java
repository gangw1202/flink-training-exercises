 ...
 *
 * Given a location that is broadcast, the goal of this exercise is to watch the stream of taxi rides, and report on the
 * taxis that complete their ride closest to the requested location.
 *
 ...
 * Some good locations:
 *
 * -74, 41 (Near, but outside the city to the NNW) -73.7781, 40.6413 (JFK Airport) -73.977664, 40.761484 (Museum of
 * Modern Art)
 */
public class NearestTaxiSolution extends ExerciseBase {
    ...
    private static class Query {
        ...
        public String toString() {
            return "Query{" + "id=" + queryId + ", longitude=" + longitude + ", latitude=" + latitude + '}';
        }
    }

    final static MapStateDescriptor queryDescriptor =
        new MapStateDescriptor<>("queries", BasicTypeInfo.LONG_TYPE_INFO, TypeInformation.of(Query.class));
    ...
    public static void main(String[] args) throws Exception {
        ...
        // add a socket source
        BroadcastStream<Query> queryStream =
            env.socketTextStream("localhost", 9999).map(new MapFunction<String, Query>() {
                @Override
                public Query map(String msg) throws Exception {
                    String[] parts = msg.split(",\\s*");
                    return new Query(Float.valueOf(parts[0]), // longitude
                        Float.valueOf(parts[1])); // latitude
                }
            }).broadcast(queryDescriptor);

        DataStream<Tuple3<Long, Long, Float>> reports =
            rides.keyBy((TaxiRide ride) -> ride.taxiId).connect(queryStream).process(new QueryFunction());
    ...
    public static class ClosestTaxi
        ...
        public void open(Configuration parameters) throws Exception {
            ValueStateDescriptor<Tuple2<Long, Float>> descriptor = new ValueStateDescriptor<Tuple2<Long, Float>>(
                // state name
                "report",
        ...
        public void processElement(Tuple3<Long, Long, Float> report, Context ctx,
    ...
    public static class QueryFunction
        extends KeyedBroadcastProcessFunction<Long, TaxiRide, Query, Tuple3<Long, Long, Float>> {

        @Override
        public void processBroadcastElement(Query query, Context ctx, Collector<Tuple3<Long, Long, Float>> out)
            throws Exception {
        ...
        public void processElement(TaxiRide ride, ReadOnlyContext ctx, Collector<Tuple3<Long, Long, Float>> out)
            throws Exception {
            if (!ride.isStart) {
                ...
                for (Map.Entry<Long, Query> entry : entries) {
                    ...
                    final float kilometersAway =
                        (float)ride.getEuclideanDistance(query.getLongitude(), query.getLatitude());

                    out.collect(new Tuple3<>(query.getQueryId(), ride.taxiId, kilometersAway));
                }
            }
        }
    }
