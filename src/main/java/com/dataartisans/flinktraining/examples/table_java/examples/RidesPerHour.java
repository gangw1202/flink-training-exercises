...
public class RidesPerHour {
    public static void main(String[] args) throws Exception {
        ...
        Table results = tEnv.sqlQuery(
            // "SELECT TUMBLE_START(eventTime, INTERVAL '1' HOUR), isStart, count(isStart) FROM TaxiRides GROUP BY
            // isStart, TUMBLE(eventTime, INTERVAL '1' HOUR)"
            // "SELECT avg(endTime - startTime), passengerCnt FROM TaxiRides GROUP BY passengerCnt"
            "SELECT CAST (toCellId(endLon, endLat) AS VARCHAR), eventTime," + "COUNT(*) OVER ("
