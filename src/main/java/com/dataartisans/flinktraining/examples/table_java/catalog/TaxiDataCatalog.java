 ...
 * <p>
 * The following tables are provided: - TaxiRides - taxi rides by {@link TaxiRideTableSource} - TaxiFares - taxi fares
 * by {@link TaxiFareTableSource}
 */
public class TaxiDataCatalog implements ExternalCatalog {
     ...
     *
     * @param ridesFile the gzipped input file from which the taxi ride rows are read, or null to skip the TaxiRides
     *        table.
     * @param faresFile the gzipped input file from which the taxi fare rows are read, or null to skip the TaxiFares
     *        table.
    ...
    public TaxiDataCatalog(String ridesFile, String faresFile, int maxEventDelay, int servingSpeedFactor) {
        ...
        if (ridesFile != null) {
            // TaxiRides
            TaxiRides ridesDescriptor =
                new TaxiRides().path(ridesFile).maxEventDelaySecs(maxEventDelay).servingSpeedFactor(servingSpeedFactor);
            this.tables.put("TaxiRides",
                ExternalCatalogTable.builder(ridesDescriptor).supportsStreaming().inAppendMode().asTableSource());
        }

        if (faresFile != null) {
            // TaxiFares
            TaxiFares faresDescriptor =
                new TaxiFares().path(faresFile).maxEventDelaySecs(maxEventDelay).servingSpeedFactor(servingSpeedFactor);
            this.tables.put("TaxiFares",
                ExternalCatalogTable.builder(faresDescriptor).supportsStreaming().inAppendMode().asTableSource());
        }
    }
