...
/*
 * This is an example of how to sort an out-of-order stream, based on event time timestamps and watermarks, using the
 * CEP library.
 * 
...
public class Sort {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<Event> eventStream =
            env.addSource(new OutOfOrderEventSource()).assignTimestampsAndWatermarks(new TimestampsAndWatermarks());

        Pattern<Event, ?> matchEverything = Pattern.<Event>begin("any").where(new SimpleCondition<Event>() {
            @Override
        ...
        SingleOutputStreamOperator<Event> sorted =
            patternStream.sideOutputLateData(lateDataOutputTag).select(new PatternSelectFunction<Event, Event>() {
                @Override
        ...
        sorted.print();
        sorted.getSideOutput(lateDataOutputTag).map(e -> new Tuple2<>(e, "LATE"))
            .returns(Types.TUPLE(TypeInformation.of(Event.class), Types.STRING)).print();
