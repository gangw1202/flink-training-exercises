...
public class BroadcastState {
    ...
    private static class Item {
        ...
        public String toString() {
            return "Item{" + "shape=" + shape + ", color=" + color + '}';
        }
    }

    final static Class<Tuple2<Shape, Shape>> TYPED_TUPLE = (Class<Tuple2<Shape, Shape>>)(Class<?>)Tuple2.class;

    final static TupleTypeInfo<Tuple2<Shape, Shape>> TUPLE_TYPE_INFO =
        new TupleTypeInfo<>(TYPED_TUPLE, new EnumTypeInfo<>(Shape.class), new EnumTypeInfo<>(Shape.class));
    ...
    public static void main(String[] args) throws Exception {
        ...
        MapStateDescriptor<String, Tuple2<Shape, Shape>> rulesStateDescriptor =
            new MapStateDescriptor<>("RulesBroadcastState", BasicTypeInfo.STRING_TYPE_INFO, TUPLE_TYPE_INFO);

        KeyedStream<Item, Color> itemColorKeyedStream = env.fromCollection(keyedInput).rebalance() // needed to increase
                                                                                                   // the parallelism
            .map(item -> item).setParallelism(4).keyBy(item -> item.color);

        BroadcastStream<Tuple2<Shape, Shape>> broadcastRulesStream =
            env.fromCollection(rules).flatMap(new FlatMapFunction<Tuple2<Shape, Shape>, Tuple2<Shape, Shape>>() {
        ...
        DataStream<String> output = itemColorKeyedStream.connect(broadcastRulesStream).process(new MatchFunction());
    ...
    public static class MatchFunction extends KeyedBroadcastProcessFunction<Color, Item, Tuple2<Shape, Shape>, String> {
        ...
        @Override
        public void processBroadcastElement(Tuple2<Shape, Shape> value, Context ctx, Collector<String> out)
            throws Exception {
        ...
        public void processElement(Item nextItem, ReadOnlyContext ctx, Collector<String> out) throws Exception {
            ...
            for (Map.Entry<String, Tuple2<Shape, Shape>> entry : ctx.getBroadcastState(broadcastStateDescriptor)
                .immutableEntries()) {
