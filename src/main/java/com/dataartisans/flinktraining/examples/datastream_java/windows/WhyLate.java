 ...
 * 
 * (2) There are several different ways to fix this job so that it never has late events, even when
 * PARTITIONS_PER_INSTANCE is greater than 1. How many solutions can you find, and which approach seems most
 * appropriate?
 * 
 */
...
public class WhyLate {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<Tuple2<String, Integer>> windowOperator =
            events.assignTimestampsAndWatermarks(new TimestampsAndWatermarks()).keyBy(e -> e.userId)
                .window(TumblingEventTimeWindows.of(Time.seconds(1))).sideOutputLateData(lateDataTag)
        ...
        windowOperator.getSideOutput(lateDataTag).windowAll(TumblingProcessingTimeWindows.of(Time.seconds(1)))
            .process(new CountLateEvents())
            .map(i -> new Tuple3<String, Integer, String>("LATE", i, Instant.now().toString()))
            .returns(Types.TUPLE(Types.STRING, Types.INT, Types.STRING)).print();
    ...
    private static class CountEventsPerUser
        extends ProcessWindowFunction<Event, Tuple2<String, Integer>, String, TimeWindow> {
        @Override
        public void process(String key, Context context, Iterable<Event> iterable,
    ...
    private static class CountLateEvents extends ProcessAllWindowFunction<Event, Integer, TimeWindow> {
        @Override
        public void process(Context context, Iterable<Event> iterable, Collector<Integer> out) throws Exception {
