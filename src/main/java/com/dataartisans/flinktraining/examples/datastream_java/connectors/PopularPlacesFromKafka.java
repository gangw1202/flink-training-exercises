 ...
 *
 * The task of the exercise is to identify every five minutes popular areas where many taxi rides arrived or departed in
 * the last 15 minutes. The input is read from a Kafka topic that containes cleansed taxi rides.
 *
 */
public class PopularPlacesFromKafka {
    ...
    public static void main(String[] args) throws Exception {
        ...
        FlinkKafkaConsumer011<TaxiRide> consumer =
            new FlinkKafkaConsumer011<>("cleansedRides", new TaxiRideSchema(), kafkaProps);
    ...
    /**
     * Assigns timestamps to TaxiRide records. Watermarks are a fixed time interval behind the max timestamp and are
    ...
    /**
     * Maps taxi ride to grid cell and event type. Start records use departure location, end record use arrival
     * location.
     */
     ...
     */
    public static class RideCounter implements WindowFunction<Tuple2<Integer, Boolean>, // input type
        ...
        TimeWindow> // window type
    {
        ...
        @Override
        public void apply(Tuple key, TimeWindow window, Iterable<Tuple2<Integer, Boolean>> gridCells,
    ...
    public static class GridToCoordinates
        implements MapFunction<Tuple4<Integer, Long, Boolean, Integer>, Tuple5<Float, Float, Long, Boolean, Integer>> {

        @Override
        public Tuple5<Float, Float, Long, Boolean, Integer> map(Tuple4<Integer, Long, Boolean, Integer> cellCount)
            throws Exception {

            return new Tuple5<>(GeoUtils.getGridCellCenterLon(cellCount.f0),
                GeoUtils.getGridCellCenterLat(cellCount.f0), cellCount.f1, cellCount.f2, cellCount.f3);
        }
    }
