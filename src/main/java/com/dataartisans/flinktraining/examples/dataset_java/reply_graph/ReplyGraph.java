...
/**
 * Java reference implementation for the "Reply Graph" exercise of the Flink training. The task of the exercise is to
 * enumerate the reply connection between two email addresses in Flink's developer mailing list and count the number of
 * connections between two email addresses.
 *
...
public class ReplyGraph {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataSet<Tuple3<String, String, String>> mails = env.readCsvFile(input)
            .lineDelimiter(MBoxParser.MAIL_RECORD_DELIM).fieldDelimiter(MBoxParser.MAIL_FIELD_DELIM)
            // messageId at position 0, sender at 2, reply-to at 5
            .includeFields("101001").types(String.class, String.class, String.class);

        // extract email addresses and filter out mails from bots
        DataSet<Tuple3<String, String, String>> addressMails = mails.map(new EmailExtractor())
            .filter(new ExcludeEmailFilter("git@git.apache.org")).filter(new ExcludeEmailFilter("jira@apache.org"));

        // construct reply connections by joining on messageId and reply-To
        DataSet<Tuple2<String, String>> replyConnections =
            addressMails.join(addressMails).where(2).equalTo(0).projectFirst(1).projectSecond(1);

        // count reply connections for each pair of email addresses
        replyConnections.groupBy(0, 1).reduceGroup(new ConnectionCounter()).print();
