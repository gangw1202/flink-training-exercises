...
/**
 * This SourceFunction generates a data stream of TaxiRide records which are read from a gzipped input file. Each record
 * has a time stamp and the input file must be ordered by this time stamp.
 *
 * In order to simulate a realistic stream source, the SourceFunction serves events proportional to their timestamps. In
 ...
 *
 * The serving speed of the SourceFunction can be adjusted by a serving speed factor. A factor of 60.0 increases the
 * logical serving time by a factor of 60, i.e., events of one minute (60 seconds) are served in 1 second.
 *
 * This SourceFunction is an EventSourceFunction and does continuously emit watermarks. Hence it is able to operate in
 * event time mode which is configured as follows:
 *
...
public class TaxiRideSource implements SourceFunction<TaxiRide> {
    ...
    /**
     * Serves the TaxiRide records from the specified and ordered gzipped input file. Rides are served exactly in order
     * of their time stamps at the speed at which they were originally generated.
     *
    ...
    /**
     * Serves the TaxiRide records from the specified and ordered gzipped input file. Rides are served exactly in order
     * of their time stamps in a serving speed which is proportional to the specified serving speed factor.
     *
     * @param dataFilePath The gzipped input file from which the TaxiRide records are read.
    ...
    /**
     * Serves the TaxiRide records from the specified and ordered gzipped input file. Rides are served out-of time stamp
    ...
    private void generateUnorderedStream(SourceContext<TaxiRide> sourceContext) throws Exception {
        long servingStartTime = System.currentTimeMillis();
        long dataStartTime;

        Random rand = new Random(7452);
        PriorityQueue<Tuple2<Long, Object>> emitSchedule =
            new PriorityQueue<>(32, (Tuple2<Long, Object> o1, Tuple2<Long, Object> o2) -> o1.f0.compareTo(o2.f0));
        ...
        while (emitSchedule.size() > 0 || reader.ready()) {
            ...
            // while there is a ride and no ride in schedule OR
            // not enough rides in schedule
            while (ride != null
