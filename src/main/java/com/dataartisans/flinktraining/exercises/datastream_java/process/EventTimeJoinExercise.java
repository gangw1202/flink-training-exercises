...
/*
 * This is an event-time-based enrichment join implemented with a CoProcessFunction, used to enrich a stream of
 * financial Trades with Customer data.
 *
 * When we receive a Trade we want to join it with the newest Customer data that was knowable at the time of the Trade.
 * This means that we ignore Customer data with timestamps after the the timestamp of the trade. And we wait until the
 * current watermark reaches the timestamp of the trade before doing the join, so that relevant Customer data will have
 * arrived.
 *
 * Assuming the watermarking is correct, and there is no late data, this will behave deterministically.
 *
 * To keep this example simpler, the implementation assumes only one Trade or Customer update per customerId, per
 * timestamp (typically, per millisecond).
 *
 ...
 *
 * (2) Implement a low-latency variant that immediately joins with available Customer data, if any, and then emits an
 * updated result if a Customer update arrives.
 *
...
public class EventTimeJoinExercise {
    public static void main(String[] args) throws Exception {
        ...
        DataStream<EnrichedTrade> joinedStream = tradeStream.keyBy("customerId")
            .connect(customerStream.keyBy("customerId")).process(new EventTimeJoinFunction());
    ...
    public static class EventTimeJoinFunction extends CoProcessFunction<Trade, Customer, EnrichedTrade> {
        ...
        public void open(Configuration config) {
            MapStateDescriptor tDescriptor = new MapStateDescriptor<Long, Trade>("tradeBuffer",
                TypeInformation.of(Long.class), TypeInformation.of(Trade.class));
            tradeMap = getRuntimeContext().getMapState(tDescriptor);

            MapStateDescriptor cDescriptor = new MapStateDescriptor<Long, Customer>("customerBuffer",
                TypeInformation.of(Long.class), TypeInformation.of(Customer.class));
            customerMap = getRuntimeContext().getMapState(cDescriptor);
        }

        @Override
        public void processElement1(Trade trade, Context context, Collector<EnrichedTrade> out) throws Exception {
        ...
        @Override
        public void onTimer(long t, OnTimerContext context, Collector<EnrichedTrade> out) throws Exception {
