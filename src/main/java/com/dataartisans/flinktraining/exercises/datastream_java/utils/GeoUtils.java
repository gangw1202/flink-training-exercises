...
public class GeoUtils {
     ...
     * start is located.
     *
     * The angle describes in which direction the destination is located from the start, i.e., 0° -> East, 90° -> South,
     * 180° -> West, 270° -> North
     *
     ...
     */
    public static int getDirectionAngle(float startLon, float startLat, float destLon, float destLat) {
    ...
    public static class ToCoords extends ScalarFunction {
        public Tuple2<Float, Float> eval(int cellId) {
            return Tuple2.of(GeoUtils.getGridCellCenterLon(cellId), GeoUtils.getGridCellCenterLat(cellId));
        }
