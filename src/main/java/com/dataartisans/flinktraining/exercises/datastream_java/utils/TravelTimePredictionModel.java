...
/**
 * TravelTimePredictionModel provides a very simple regression model to predict the travel time to a destination
 * location depending on the direction and distance of the departure location.
 *
 * The model builds for multiple direction intervals (think of it as north, north-east, east, etc.) a linear regression
 * model (Apache Commons Math, SimpleRegression) to predict the travel time based on the distance.
 *
 * NOTE: This model is not mean for accurate predictions but rather to illustrate Flink's handling of operator state.
 *
 */
public class TravelTimePredictionModel {
     ...
     * @return A prediction of the time that the taxi will be traveling or -1 if no prediction is possible, yet.
     */
